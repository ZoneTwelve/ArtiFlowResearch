...
def main():
    ...
    # Set up a pipeline for inference
    ...
    # Process in batches (you can adjust the batch size depending on your available memory)
    batch_size = 50  # Adjust as necessary for your machine
    results = []
    ...
    for idx in tqdm(range(0, len(test_data), batch_size), desc="Processing Batches", total=(length_data // batch_size) + 1):
        ...
        outputs = inference_pipeline(articles, max_length=args.max_output_length, num_beams=1, do_sample=True)

        # Process each generated title
        for i, sample in enumerate(batch):
            generated_title = outputs[i]['generated_text']
